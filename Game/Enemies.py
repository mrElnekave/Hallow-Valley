    ...
    def pos_validate(self):
        ...
        if self.rect.center[0] < 0:  # Moving off left of screen
            if self.chunk[0] == -1 or self.chunk[0] == 0:  # If in subchunk or leftmost chunk and boss fights.
                ...
                objects.reports_on and objects.update_log.addMessage(f'REPORT: Current chunk is {self.chunk}')
...
class Ghost(Enemy):
    ...
    def render(self):
        ...
        pygame.draw.rect(objects.display, (0, 0, 0),
                         pygame.Rect(self.rect.left, self.rect.top - self.rect[3] * .1, self.rect[2],
                                     self.rect[3] * .1))
        pygame.draw.rect(objects.display, (255, 0, 0), pygame.Rect(self.rect.left, self.rect.top - self.rect[3] * .1,
                                                                   self.rect[2] * (self.health / self.maxHealth),
                                                                   self.rect[3] * .1))
    ...
    def update(self):
        ...
        if self.rect.colliderect(objects.player.rect):
            ...
            while self.rect.colliderect(objects.player.rect):
                self.rect.center = (
                random.randint(100, objects.WINDOWWIDTH - 100), random.randint(100, objects.WINDOWHEIGHT - 100))
        if self.health <= 0:
            ...
            if objects.currentChunk.location[0] + 1 != len(objects.chunks):
                ...
                objects.currentChunk.contents.append(
                    MapClasses.QuestionCube(self.rect.center))  # TODO: bring them back
            return
...
class FireGhostBoss(Enemy):
    ...
    def update(self):
        ...
        self.rect.center = (
        self.rect.centerx + self.speed * math.cos(self.angle), self.rect.centery - self.speed * math.sin(self.angle))
...
class LightningGhostBoss(Enemy):
    ...
    def update(self):
        ...
        # objects.update_log.addMessage(self.counter)
        ...
        if self.health <= 0:
            ...
            objects.reports_on and objects.update_log.addMessage(
                "Ability Information: The electrodash ability allows you to dash quickly to a spot on the screen, doing damage to everything in your path and not taking damage at all. This ability uses up 25 ghost energy per use. Press 4 to switch to the electrodash ability from another ability.")
...
class PoisonGhostBoss(Enemy):
    ...
    def update(self):
        ...
        if self.counter < 6:
            ...
            objects.currentChunk.contents.append(
                PoisonDrop(self.rect.center, (random.randint(100, 400), random.randint(100, 400))))
            objects.currentChunk.contents.append(
                PoisonDrop(self.rect.center, (random.randint(100, 400), random.randint(100, 400))))
        ...
        if self.counter == objects.framerate * self.waitTime + 5:
            ...
            # objects.update_log.addMessage(self.rect.center)
        else:
            self.counter += 1
        ...
        if self.health <= 0:
            ...
            objects.reports_on and objects.update_log.addMessage(
                "Ability Information: The poison field ability deals damage over time to enemies near you. This ability uses up 25 ghost energy per use. Press 5 to switch to the poison field ability from another ability.")
...
class SummoningGhostBoss(Enemy):
    ...
    def update(self):
        ...
        if self.health <= 0:
            ...
            objects.reports_on and objects.update_log.addMessage(
                "Ability Information: The summoning ability summons a ghost that follows your mouse around the screen. This ability uses up 25 ghost energy per use. Press 6 to switch to the summoning ability from another ability.")
...
class ShieldGhostBoss(Enemy):
    ...
    def update(self):
        ...
        if self.health <= 0:
            ...
            objects.update_log.addMessage(
                "Ability Information: The magical shield ability makes you immune to taking damage. This ability uses up 25 ghost energy per use. Press 7 to switch to the poison field ability from another ability.")
...
class LaserGhostBoss(Enemy):
    ...
    def update(self):
        ...
        if self.counter < objects.framerate * (self.waitTime - 3) and self.counter > objects.framerate:
            ...
            objects.currentChunk.contents.append(
                EnemyLaser((self.laserMoveX, self.laserMoveY), self.laserRotation, self.rect.center))
        ...
        if self.health <= 0:
            ...
            objects.update_log.addMessage(
                "Ability Information: The laser arrow ability allows you to shoot a large arrow that passes through enemies, dealing high damage. This ability uses up 25 ghost energy per use. Press 8 to switch to the laser arrow ability from another ability.")
...
class WaterGhostBoss:
    ...
    def update(self):
        ...
        if self.health <= 0:
            ...
            objects.update_log.addMessage(
                "Ability Information: The wave ability launches 4 large waves in different directions. These waves deal damage over time to enemies that they collide with, and pull/push non-boss enemies along with them (dealing more damage). This ability uses up 25 ghost energy per use. Press 9 to switch to the wave ability from another ability.")
...
class FinalBossGhost(Enemy):
    def __init__(self):
        ...
        self.images = [self.neutralImage, self.fireImage, self.iceImage, self.lightningImage, self.poisonImage,
                       self.summoningImage, self.shieldImage, self.laserImage, self.waterImage]
        self.currentState = 0
    ...
    def update(self):
        ...
        if self.currentState == 1:  # Fire
            ...
            # Moving
            self.rect.center = (
            self.rect.centerx + 4 * math.cos(self.angle), self.rect.centery - 4 * math.sin(self.angle))
        ...
        if self.currentState == 2:  # Ice
            ...
            # Moving
            self.rect.center = (
            self.rect.centerx + 5 * math.cos(self.iceDir), self.rect.centery + 5 * math.sin(self.iceDir))
        ...
        if self.health <= 0:
            ...
            objects.update_log.addMessage("REPORT: You have defeated the dark ghost.")
            objects.update_log.addMessage("GAME COMPLETE!")
...
class PoisonDrop:
    ...
    def update(self):
        self.rect = self.rect.move(self.speed * math.cos(math.radians(self.direction)),
